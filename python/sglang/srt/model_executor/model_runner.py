...
class ModelRunner(ModelRunnerKVCacheMixin):
    ...
    def __init__(
        ...
        if _target in _all_buckets:
            ...
            self._prof_buckets = {
                _target: {"lo": cfg["lo"], "hi": cfg["hi"], "steps": cfg["steps"],
                          "done": False, "active": False, "cnt": 0},
            }
            logging.info(f"[PROF] 目标桶={_target}  avg_seq_len 范围=[{cfg['lo']}, {cfg['hi']})")
        else:
            self._prof_buckets = {}   # 未设置或非法值 → 不采集
            if _target:
                logging.warning(f"[PROF] PROF_BUCKET={_target!r} 不合法，跳过 profiling")
    ...
    def _forward_raw(
        ...
        if forward_batch.batch_size > 0:
            if forward_batch.forward_mode.is_decode():
                ...
                if self.forward_pass_id > self._STATS_WARMUP:
                    seq_lens_sum = int(forward_batch.seq_lens_sum)
                    self._size_stats.setdefault("extend_prefill", []).append(seq_lens_sum)
        # ─────────────────────────────────────────────────────────────────────
